...
import borsch.services.Interfaces.RecipeService;
...
public class RecipeServiceImpl implements RecipeService {
    ...
    @Override
    public Map<Long, UserInfoData> provideMapFinalUsers(Long idRecipe) {

        try {
            Optional<Recipe> recipe = recipeRepository.findById(idRecipe);

            Map<Long, UserInfoData> result = new HashMap<>();
            ...
            if (recipe.isPresent()){
                ...
                recipe
                        ...
                        .parallelStream()
                        .forEach(p -> result
                                .put(p.getId(),p.getUserInfo().getUserInfoData()));
